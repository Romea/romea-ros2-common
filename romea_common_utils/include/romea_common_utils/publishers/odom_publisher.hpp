...
//-----------------------------------------------------------------------------
template<typename DataType, typename NodeType>
void OdomPublisher<DataType, NodeType>::publish(const rclcpp::Time & stamp, const DataType & data)
{
...
//-----------------------------------------------------------------------------
template<typename DataType, typename NodeType>
std::shared_ptr<OdomPublisher<DataType, NodeType>> make_odom_publisher(
  std::shared_ptr<NodeType> node,
  const std::string & topic_name,
  const std::string & frame_id,
  const std::string & child_frame_id,
  ...
  const bool & activated)
{
  ...
  return std::make_shared<Publisher>(node, topic_name, frame_id, child_frame_id, qos, activated);
}
